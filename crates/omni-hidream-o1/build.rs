//! Compiles `kernels/hidream.cu` to the cubin the model's kern manifest pins.
...
fn main() {
    ...
    let status = Command::new(&nvcc)
        ...
        .arg("-o")
        .arg(out.join("hidream.cubin"))
        .arg("kernels/hidream.cu")
        .status()
        .unwrap_or_else(|e| panic!("running {}: {e}", nvcc.display()));
    assert!(status.success(), "nvcc failed on kernels/hidream.cu");
}
