...
fn duplex_sessions_against_the_live_sim_run_clean() {
    ...
    let out = std::env::temp_dir().join(format!("omni-bench-duplex-{}.json", std::process::id()));
    let status = std::process::Command::new(env!("CARGO_BIN_EXE_omni-bench"))
        .args(["duplex", "--base-url", &format!("http://{addr}"), "--sessions", "4", "--seconds", "1.5"])
        .arg("--out")
        .arg(&out)
        .status()
        .unwrap();
    ...
    assert_eq!((report["levels"][0]["sessions"].as_u64(), report["levels"][0]["failed"].as_u64()), (Some(4), Some(0)));
